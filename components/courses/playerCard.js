...
<div className="flex w-[90vw] flex-col mt-10 items-center overflow-y-scroll bg-black min-h-screen">
  {courseData.map((course, index) => (
    <div key={index} className="relative w-[90vw] mb-3 flex flex-col min-h-[58px] border border-purple-500 rounded-lg bg-purple-800 overflow-hidden transition-all duration-300">
      <input
        hidden
      ...
      <ul
        className={`list ${openStates[index] ? 'opacity-100 translate-y-0 max-h-80' : 'opacity-0 translate-y-[3rem] mt-[-100%]'} overflow-hidden auto-scroll gap-4 p-4 transition-all duration-500 ease-out flex items-center flex-col justify-between bg-black`}
        role="list"
      >
        {subCourseData && subCourseData.data && subCourseData.data.map((subCourse, index) => (
          <li className="listitem w-[95%]" role="listitem" key={index} onClick={() => setShowVideo(subCourse.video_id)}>
